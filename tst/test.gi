...
getRandomPc := function(G)
	...
	if not IsPcGroup(G) then G := Image(IsomorphismPcGroup(G)); fi;
		...
		repeat
			...
			if not Order(el) mod Size(Image(hom))=0 then Error("mhmm");
			fi;
	...
	repeat
		...
		if IsSOTAvailable(n) then
			...
			for i in [1..nr] do
				...
				if not ForAll(copies,x->IdSOTGroup(x)=[n,i]) then Error("SOT ID perm", [n,i]);
				fi;
			od;
			...
			if gap then
				...
				if not Size(gapid) = NumberSmallGroups(n) then Error("gap nr");
				fi;
				if not IsDuplicateFreeList(gapid) then Error("gap id");
				fi;
				new := List([1..nr],x->IdSOTGroup(SmallGroup(n,x)));
				if not IsDuplicateFreeList(new) then Error("SOT id");
				fi;
...
SOTRec.testbyRandomIsomorphismTest := function(x)
	...
	if not IsSOTAvailable(x) then Error("Wrong input."); fi;
		...
		if SmallGroupsAvailable(x) then
			if not Size(sot)=NumberSmallGroups(x) then
				Error("Revise enumeration.");
			fi;
			if not AsSet(List(sot,IdSmallGroup))=AsSet(List([1..NumberSmallGroups(x)],i->[x,i])) then
				Error("Gulp! Construction contains duplicate(s)!");
			fi;
		else
			Print("start gap\n");
			tg := Runtime();
			...
			if Size(cd)<Size(sot) then
				Error("random is");
			else Print("ok\n");
			fi;
		fi;
		if not ForAll(sot,i->Order(i)=x) then
			Error("order");
		fi;
		return t;
	end;
