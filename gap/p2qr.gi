deltaDivisibility := function(x, y)
  local w;
    if x mod y = 0 then w := 1;
    else w := 0; fi;
  return w;
  end;

deltafunction := function(x, y)
  local w;
    if x = y then w := 1;
    else w := 0; fi;
  return w;
  end;

groupofunitsP2 := function(p)
  local l;
    l := Filtered([1..p^2], x->not x mod p = 0);
    return l;
  end;
############################################################################
msg.QthRootGL2P := function(p, q)
  	local a, b;
  	if not Gcd(p,q)=1 or not (p^2-1) mod q = 0 then
  	 Error("Arguments has to be coprime, where q divides (p^2 - 1).\n");
  	 else
  	 a := PrimitiveElement(GF(p^2));
  	 b := a^((p^2-1)/q);
  	fi;
     return [ [0, 1], [-b^(p+1), b+b^p] ] * One(GF(p));
   end;
############################################################################
InstallGlobalFunction( allGroupsP2QR, function(n)
  local fac, primefac, p, q, r, coll, case1, case2, case3, case4, case5, case6, case7, case8, s;
    s := [];
    fac := Factors(n);
    if not Length(fac) = 4 or not Length(Collected(fac)) = 3 then
      Error("Argument must be of the form of p^2qr"); fi;
      primefac := function(n)
        local i, j, tmp;
          tmp := [];
          for i in Collected(fac) do
            if i[2] = 2 then j := i[1];
            elif i[2] = 1 then Add(tmp, i[1]);
            fi;
          od;
          Sort(tmp);
          Add(tmp, j);
          return tmp;
        end;
    p := primefac(n)[3]; q := primefac(n)[1]; r := primefac(n)[2];
############ add abelian groups in:
    Append(s, [AbelianGroup([n]), AbelianGroup([p*q*r, p])]);
############ case 1: nonabelian and Fitting subgroup has order r -- unique isomorphism type iff p^2q | (r - 1)
      case1 := function(p, q, r)
        local a, b, c, d, coll, G;
          a := Z(r);
          b := a^((r-1)/(p^2*q));
          c := b^p;
          d := c^p;
          coll := FromTheLeftCollector(4);
          SetRelativeOrder(coll, 1, p);
          SetRelativeOrder(coll, 2, p);
          SetRelativeOrder(coll, 3, q);
          SetRelativeOrder(coll, 4, r);
          SetPower(coll, 1, [2, 1]);
          SetPower(coll, 2, [3, 1]);
          SetConjugate(coll, 4, 1, [4, Int(b)]);
          SetConjugate(coll, 4, 2, [4, Int(c)]);
          SetConjugate(coll, 4, 3, [4, Int(d)]);
          G := PcpGroupByCollector(coll);
        return PcpGroupToPcGroup(G:FreeGroupFamilyType:="syllable");
      end;

    if (r - 1) mod (p^2*q) = 0 then Add(s, case1(p, q, r)); fi;
############ case 2: nonabelian and Fitting subgroup has order qr -- p^2 | (q - 1)(r - 1) or p | (q - 1)(r - 1)
      case2 := function(p, q, r)
        local  list, G1, G2, G3, G4, G5, G6, i;
         list := [];
        ##
        G1 := function(p, q, r) ## p^2 | (q - 1) and G \cong (C_{p^2} \ltimes C_q) \times C_r
          local a, b, c, d, s, t, x, y, alpha, beta, coll, G;
            a := Z(q);
            s := a^((q-1)/(p^2));
            coll := FromTheLeftCollector(4);
            SetRelativeOrder(coll, 1, p);
            SetRelativeOrder(coll, 2, p);
            SetPower(coll, 1, [2, 1]);
            SetRelativeOrder(coll, 3, q);
            SetRelativeOrder(coll, 4, r);
            SetConjugate(coll, 3, 1, [3, Int(s)]);
            SetConjugate(coll, 3, 2, [3, Int(s^p)]);
            G := PcpGroupByCollector(coll);
          return PcpGroupToPcGroup(G:FreeGroupFamilyType:="syllable");
        end;
        ##
        if (q - 1) mod (p^2) = 0 then Add( list, G1(p, q, r)); fi;
        ##
        G2 := function(p, q, r, k) ## p^2 | (q - 1), p | (r - 1), and G \cong C_{p^2} \ltimes (C_q \times C_r)
          local a, b, c, d, s, t, x, y, alpha, beta, coll, G;
            a := Z(q);
            b := Z(r);
            s := a^((q-1)/(p^2));
            t := b^(k*(r-1)/p);
            coll := FromTheLeftCollector(4);
            SetRelativeOrder(coll, 1, p);
            SetRelativeOrder(coll, 2, p);
            SetPower(coll, 1, [2, 1]);
            SetRelativeOrder(coll, 3, q);
            SetRelativeOrder(coll, 4, r);
            SetConjugate(coll, 3, 1, [3, Int(s)]);
            SetConjugate(coll, 3, 2, [3, Int(s^p)]);
            SetConjugate(coll, 4, 1, [4, Int(t)]);
            G := PcpGroupByCollector(coll);
          return PcpGroupToPcGroup(G:FreeGroupFamilyType:="syllable");
        end;
        ##
        if (q - 1) mod (p^2) = 0 and (r - 1) mod p = 0 then
          for i in [1..p-1] do Add( list, G2(p, q, r, i));
          od;
        fi;
        ##
        G3 := function(p, q, r, k) ## p^2 | (q - 1), p^2 | (r - 1), and G \cong C_{p^2} \ltimes (C_q \times C_r)
          local a, b, c, d, s, t, x, y, alpha, beta, coll, G;
            a := Z(q);
            b := Z(r);
            s := a^((q-1)/(p^2));
            t := b^(k*(r-1)/(p^2));
            coll := FromTheLeftCollector(4);
            SetRelativeOrder(coll, 1, p);
            SetRelativeOrder(coll, 2, p);
            SetPower(coll, 1, [2, 1]);
            SetRelativeOrder(coll, 3, q);
            SetRelativeOrder(coll, 4, r);
            SetConjugate(coll, 3, 1, [3, Int(s)]);
            SetConjugate(coll, 3, 2, [3, Int(s^p)]);
            SetConjugate(coll, 4, 1, [4, Int(t)]);
            SetConjugate(coll, 4, 2, [4, Int(t^p)]);
            G := PcpGroupByCollector(coll);
          return PcpGroupToPcGroup(G:FreeGroupFamilyType:="syllable");
        end;
        ##
        if (q - 1) mod (p^2) = 0 and (r - 1) mod (p^2) = 0 then
          for i in groupofunitsP2(p) do Add( list, G3(p, q, r, i));
          od;
        fi;
        ##
        G4 := function(p, q, r) ## p^2 | (r - 1), and G \cong (C_{p^2} \ltimes C_r) \times C_q
          local a, b, c, d, s, t, x, y, alpha, beta, coll, G;
            b := Z(r);
            t := b^((r-1)/(p^2));
            coll := FromTheLeftCollector(4);
            SetRelativeOrder(coll, 1, p);
            SetRelativeOrder(coll, 2, p);
            SetPower(coll, 1, [2, 1]);
            SetRelativeOrder(coll, 3, q);
            SetRelativeOrder(coll, 4, r);
            SetConjugate(coll, 4, 1, [4, Int(t)]);
            SetConjugate(coll, 4, 2, [4, Int(t^p)]);
            G := PcpGroupByCollector(coll);
          return PcpGroupToPcGroup(G:FreeGroupFamilyType:="syllable");
        end;
        ##
        if (r - 1) mod (p^2) = 0 then
          Add( list, G4(p, q, r));
        fi;
        ##
        G5 := function(p, q, r, k) ## p | (q - 1), p^2 | (r - 1), and G \cong C_{p^2} \ltimes (C_q \times C_r)
          local a, b, c, d, s, t, x, y, alpha, beta, coll, G;
            a := Z(q);
            b := Z(r);
            s := a^(k*(q-1)/p);
            t := b^((r-1)/(p^2));
            coll := FromTheLeftCollector(4);
            SetRelativeOrder(coll, 1, p);
            SetRelativeOrder(coll, 2, p);
            SetPower(coll, 1, [2, 1]);
            SetRelativeOrder(coll, 3, q);
            SetRelativeOrder(coll, 4, r);
            SetConjugate(coll, 3, 1, [3, Int(s)]);
            SetConjugate(coll, 4, 1, [4, Int(t)]);
            SetConjugate(coll, 4, 2, [4, Int(t^p)]);
            G := PcpGroupByCollector(coll);
          return PcpGroupToPcGroup(G:FreeGroupFamilyType:="syllable");
        end;
        ##
        if (r - 1) mod (p^2) = 0 and (q - 1) mod p = 0 then
          for i in [1..p-1]
            do Add( list, G5(p, q, r, i));
          od;
        fi;
        ##
        G6 := function(p, q, r) ## p | (q - 1), p | (r - 1), and G \cong (C_p \ltimes C_q) \times (C_p \ltimes C_r)
          local a, b, c, d, s, t, x, y, alpha, beta, coll, G;
            a := Z(q);
            b := Z(r);
            s := a^((q-1)/p);
            t := b^((r-1)/p);
            coll := FromTheLeftCollector(4);
            SetRelativeOrder(coll, 1, p);
            SetRelativeOrder(coll, 2, p);
            SetRelativeOrder(coll, 3, q);
            SetRelativeOrder(coll, 4, r);
            SetConjugate(coll, 3, 1, [3, Int(s)]);
            SetConjugate(coll, 4, 2, [4, Int(t)]);
            G := PcpGroupByCollector(coll);
          return PcpGroupToPcGroup(G:FreeGroupFamilyType:="syllable");
        end;
        ##
        if (r - 1) mod p = 0 and (q - 1) mod p = 0 then
          Add( list, G6(p, q, r));
        fi;
        ##
        return  list;
      end;
############
    Append(s, case2(p, q, r));
############ case 3: nonabelian and Fitting subgroup has order p^2 -- qr | (p^2 - 1)
      case3 := function(p, q, r)
        local  list, G1, G2, G3, G4, G5, G6, G7, G8, G9, G10, G11, G12, i, j;
          list := [];
          G1 := function(p, q, r) ## qr | (p - 1) and G \cong C_{qr} \ltimes C_{p^2}
            local a, b, c, d, s, t, ii, qq, iii, qqq, coll, G;
              a := ZmodnZObj(Int(Z(p)),p^2);
              if not a^(p-1) = ZmodnZObj(1,p^2) then b := a; else b := a+1; fi;
              s := Int(b^((p^2-p)/q));
              t := Int(b^((p^2-p)/r));
              ii := s mod p;
              qq := (s - ii)/p;
              iii := t mod p;
              qqq := (t - iii)/p;
              coll := FromTheLeftCollector(4);
              SetRelativeOrder(coll, 1, q);
              SetRelativeOrder(coll, 2, r);
              SetRelativeOrder(coll, 3, p);
              SetRelativeOrder(coll, 4, p);
              SetPower(coll, 3, [4, 1]);
              SetConjugate(coll, 3, 1, [3, ii, 4, qq]);
              SetConjugate(coll, 3, 2, [3, iii, 4, qqq]);
              SetConjugate(coll, 4, 1, [4, ii]);
              SetConjugate(coll, 4, 2, [4, iii]);
              G := PcpGroupByCollector(coll);
            return PcpGroupToPcGroup(G:FreeGroupFamilyType:="syllable");
          end;
          ##
          if (p - 1) mod q = 0 and (p - 1) mod r = 0
            then Add( list, G1(p, q, r));
          fi;
          ##
          G2 := function(p, q, r) ## qr | (p - 1) and G \cong (C_q \ltimes C_p) \times (C_r \ltimes C_p)
            local a, b, c, d, s, t, x, y, alpha, beta, coll, G;
              a := Z(p);
              s := a^((p-1)/q);
              t := a^((p-1)/r);
              coll := FromTheLeftCollector(4);
              SetRelativeOrder(coll, 1, q);
              SetRelativeOrder(coll, 2, r);
              SetRelativeOrder(coll, 3, p);
              SetRelativeOrder(coll, 4, p);
              SetConjugate(coll, 3, 1, [3, Int(s)]);
              SetConjugate(coll, 4, 2, [4, Int(t)]);
              G := PcpGroupByCollector(coll);
            return PcpGroupToPcGroup(G:FreeGroupFamilyType:="syllable");
          end;
          ##
          if (p - 1) mod q = 0 and (p - 1) mod r = 0
            then Add( list, G2(p, q, r));
          fi;
          ##
          G3 := function(p, q, r) ## qr | (p - 1) and G \cong (C_{qr} \ltimes C_p) \times C_p
            local a, b, c, d, s, t, x, y, alpha, beta, coll, G;
              a := Z(p);
              s := a^((p-1)/q);
              t := a^((p-1)/r);
              coll := FromTheLeftCollector(4);
              SetRelativeOrder(coll, 1, q);
              SetRelativeOrder(coll, 2, r);
              SetRelativeOrder(coll, 3, p);
              SetRelativeOrder(coll, 4, p);
              SetConjugate(coll, 3, 1, [3, Int(s)]);
              SetConjugate(coll, 3, 2, [3, Int(t)]);
              G := PcpGroupByCollector(coll);
            return PcpGroupToPcGroup(G:FreeGroupFamilyType:="syllable");
          end;
          ##
          if (p - 1) mod q = 0 and (p - 1) mod r = 0
            then Add( list, G3(p, q, r));
          fi;
          ##
          G4 := function(p, q, r, k) ## qr | (p - 1) and G \cong C_{qr} \ltimes C_p^2
            local a, b, c, d, s, t, x, y, alpha, beta, coll, G;
              a := Z(p);
              s := a^((p-1)/q);
              t := a^((p-1)/r);
              coll := FromTheLeftCollector(4);
              SetRelativeOrder(coll, 1, q);
              SetRelativeOrder(coll, 2, r);
              SetRelativeOrder(coll, 3, p);
              SetRelativeOrder(coll, 4, p);
              SetConjugate(coll, 3, 1, [3, Int(s)]);
              SetConjugate(coll, 3, 2, [3, Int(t)]);
              SetConjugate(coll, 4, 1, [4, Int(s^i)]);
              G := PcpGroupByCollector(coll);
            return PcpGroupToPcGroup(G:FreeGroupFamilyType:="syllable");
          end;
          ##
          if (p-1) mod (q*r) = 0 then
            for i in [1..q-1] do
              Add( list, G4(p, q, r, i));
            od;
          fi;
          ##
          G5 := function(p, q, r, k) ## qr | (p - 1) and G \cong C_{qr} \ltimes C_p^2
            local a, b, c, d, s, t, x, y, alpha, beta, coll, G;
              a := Z(p);
              s := a^((p-1)/q);
              t := a^((p-1)/r);
              coll := FromTheLeftCollector(4);
              SetRelativeOrder(coll, 1, q);
              SetRelativeOrder(coll, 2, r);
              SetRelativeOrder(coll, 3, p);
              SetRelativeOrder(coll, 4, p);
              SetConjugate(coll, 3, 1, [3, Int(s)]);
              SetConjugate(coll, 3, 2, [3, Int(t)]);
              SetConjugate(coll, 4, 2, [4, Int(t^i)]);
              G := PcpGroupByCollector(coll);
            return PcpGroupToPcGroup(G:FreeGroupFamilyType:="syllable");
          end;
          ##
          if (p-1) mod (q*r) = 0 then
            for i in [1..r-1] do
              Add( list, G5(p, q, r, i));
            od;
          fi;
          ##
          G6 := function(p, q, r, k, l) ## qr | (p - 1) and G \cong C_{qr} \ltimes C_p^2
            local a, b, c, d, s, t, x, y, alpha, beta, coll, G;
              a := Z(p);
              alpha := Z(q);
              beta := Z(r);
              s := a^((p-1)/q);
              t := a^((p-1)/r);
              x := Int(alpha^k);
              y := Int(beta^l);
              coll := FromTheLeftCollector(4);
              SetRelativeOrder(coll, 1, q);
              SetRelativeOrder(coll, 2, r);
              SetRelativeOrder(coll, 3, p);
              SetRelativeOrder(coll, 4, p);
              SetConjugate(coll, 3, 1, [3, Int(s)]);
              SetConjugate(coll, 3, 2, [3, Int(t)]);
              SetConjugate(coll, 4, 1, [4, Int(s^x)]);
              SetConjugate(coll, 4, 2, [4, Int(t^y)]);
              G := PcpGroupByCollector(coll);
            return PcpGroupToPcGroup(G:FreeGroupFamilyType:="syllable");
          end;
          ##
          if (p-1) mod (q*r) = 0 and q > 2 then
            for i in [0..(q-1)/2] do
              for j in [0..(r-1)/2] do
                Add( list, G6(p, q, r, i, j));
              od;
            od;
          fi;
          ##
          ##
          if (p-1) mod (q*r) =0 and q > 2 then
            for i in [1..(q-3)/2] do
              for j in [(r+1)/2..(r-1)] do
                Add( list, G6(p, q, r, i, j));
              od;
            od;
          fi;
          ##
          ##
          if (p-1) mod (q*r) = 0 and q = 2 then
              for j in [0..(r-1)/2] do
                Add( list, G6(p, q, r, 0, j));
              od;
          fi;
          ##
          G7 := function(p, q, r) ##q = 2, r | (p - 1), and G \cong (C_2 \ltimes C_r) \ltimes C_p^2
            local a, b, c, d, s, t, coll, G;
              a := Z(p);
              t := a^((p-1)/r);
              coll := FromTheLeftCollector(4);
              SetRelativeOrder(coll, 1, q);
              SetRelativeOrder(coll, 2, r);
              SetRelativeOrder(coll, 3, p);
              SetRelativeOrder(coll, 4, p);
              SetConjugate(coll, 2, 1, [2, r-1]);
              SetConjugate(coll, 3, 1, [4, 1]);
              SetConjugate(coll, 3, 2, [3, Int(t)]);
              SetConjugate(coll, 4, 1, [3, 1]);
              SetConjugate(coll, 4, 2, [4, Int(t^(-1))]);
              G := PcpGroupByCollector(coll);
            return PcpGroupToPcGroup(G:FreeGroupFamilyType:="syllable");
          end;
          ##
          if (p-1) mod (q*r) = 0 and q = 2 then
            Add( list, G7(p, q, r));
          fi;
          ##
          G8 := function(p, q, r) ## qr | (p + 1), q > 2, and G \cong C_{qr} \ltimes C_p^2
            local matqr, matr, coll, G;
              matqr := msg.QthRootGL2P(p, (q*r));
              matr := matqr^q;
              coll := FromTheLeftCollector(4);
              SetRelativeOrder(coll, 1, q);
              SetRelativeOrder(coll, 2, r);
              SetPower(coll, 1, [2, 1]);
              SetRelativeOrder(coll, 3, p);
              SetRelativeOrder(coll, 4, p);
              SetConjugate(coll, 3, 1, [3, Int(matqr[1][1]), 4, Int(matqr[2][1])]);
              SetConjugate(coll, 4, 1, [3, Int(matqr[1][2]), 4, Int(matqr[2][2])]);
              SetConjugate(coll, 3, 2, [3, Int(matr[1][1]), 4, Int(matr[2][1])]);
              SetConjugate(coll, 4, 2, [3, Int(matr[1][2]), 4, Int(matr[2][2])]);
              G := PcpGroupByCollector(coll);
            return PcpGroupToPcGroup(G:FreeGroupFamilyType:="syllable");
          end;
          ##
          if (p+1) mod (q*r) = 0 and q > 2 then
            Add( list, G8(p, q, r));
          fi;
          ##
          G9 := function(p, q, r) ## qr | (p + 1), q = 2, and G \cong C_{qr} \ltimes C_p^2
            local matq, matr, coll, G;
              matr := msg.QthRootGL2P(p, r);
              coll := FromTheLeftCollector(4);
              SetRelativeOrder(coll, 1, q);
              SetRelativeOrder(coll, 2, r);
              SetRelativeOrder(coll, 3, p);
              SetRelativeOrder(coll, 4, p);
              SetConjugate(coll, 3, 1, [3, p - 1]);
              SetConjugate(coll, 4, 1, [4, p - 1]);
              SetConjugate(coll, 3, 2, [3, Int(matr[1][1]), 4, Int(matr[2][1])]);
              SetConjugate(coll, 4, 2, [3, Int(matr[1][2]), 4, Int(matr[2][2])]);
              G := PcpGroupByCollector(coll);
            return PcpGroupToPcGroup(G:FreeGroupFamilyType:="syllable");
          end;
          ##
          if (p+1) mod (q*r) = 0 and q = 2 then
            Add( list, G9(p, q, r));
          fi;
          ##
          G10 := function(p, q, r) ## qr | (p + 1), q = 2, and G \cong (C_q \ltimes C_r)\ltimes C_p^2
            local matq, matr, coll, G;
              matr := msg.QthRootGL2P(p, r);
              coll := FromTheLeftCollector(4);
              SetRelativeOrder(coll, 1, q);
              SetRelativeOrder(coll, 2, r);
              SetRelativeOrder(coll, 3, p);
              SetRelativeOrder(coll, 4, p);
              SetConjugate(coll, 2, 1, [2, r-1]);
              SetConjugate(coll, 3, 1, [4, 1]);
              SetConjugate(coll, 4, 1, [3, 1]);
              SetConjugate(coll, 3, 2, [3, Int(matr[1][1]), 4, Int(matr[2][1])]);
              SetConjugate(coll, 4, 2, [3, Int(matr[1][2]), 4, Int(matr[2][2])]);
              G := PcpGroupByCollector(coll);
            return PcpGroupToPcGroup(G:FreeGroupFamilyType:="syllable");
          end;
          ##
          if (p+1) mod (q*r) = 0 and q = 2 then
            Add( list, G10(p, q, r));
          fi;
          ##
          G11 := function(p, q, r) ## q | (p - 1), r | (p + 1), and G \cong (C_q \times C_r) \ltimes C_p^2
            local a, t, matr, coll, G;
              a := Z(p);
              t := a^((p - 1)/q);
              matr := msg.QthRootGL2P(p, r);
              coll := FromTheLeftCollector(4);
              SetRelativeOrder(coll, 1, q);
              SetRelativeOrder(coll, 2, r);
              SetRelativeOrder(coll, 3, p);
              SetRelativeOrder(coll, 4, p);
              SetConjugate(coll, 3, 1, [3, Int(t)]);
              SetConjugate(coll, 4, 1, [4, Int(t)]);
              SetConjugate(coll, 3, 2, [3, Int(matr[1][1]), 4, Int(matr[2][1])]);
              SetConjugate(coll, 4, 2, [3, Int(matr[1][2]), 4, Int(matr[2][2])]);
              G := PcpGroupByCollector(coll);
            return PcpGroupToPcGroup(G:FreeGroupFamilyType:="syllable");
          end;
          ##
          if (p + 1) mod r = 0 and (p - 1) mod q = 0 and q > 2 then
            Add( list, G11(p, q, r));
          fi;
          ##
          G12 := function(p, q, r) ## q | (p + 1), r | (p - 1), and G \cong (C_q \times C_r) \ltimes C_p^2
            local a, s, matq, coll, G;
              a := Z(p);
              s := a^((p - 1)/r);
              matq := msg.QthRootGL2P(p, q);
              coll := FromTheLeftCollector(4);
              SetRelativeOrder(coll, 1, q);
              SetRelativeOrder(coll, 2, r);
              SetRelativeOrder(coll, 3, p);
              SetRelativeOrder(coll, 4, p);
              SetConjugate(coll, 3, 1, [3, Int(matq[1][1]), 4, Int(matq[2][1])]);
              SetConjugate(coll, 4, 1, [3, Int(matq[1][2]), 4, Int(matq[2][2])]);
              SetConjugate(coll, 3, 2, [3, Int(s)]);
              SetConjugate(coll, 4, 2, [4, Int(s)]);
              G := PcpGroupByCollector(coll);
            return PcpGroupToPcGroup(G:FreeGroupFamilyType:="syllable");
          end;
          ##
          if (p - 1) mod r = 0 and (p + 1) mod q = 0 and q > 2 then
            Add( list, G12(p, q, r));
          fi;
          ##
          return list;
        end;
############
      Append(s, case3(p, q, r));
############ case 4: nonabelian and Fitting subgroup has order p^2q -- r | (p - 1) or r | (p + 1)
      case4 := function(p, q, r)
        local list, G1, G2, G3, G4, i;
          list := [];
          G1 := function(p, q, r) ## r | (p - 1) and G \cong (C_r \ltimes C_{p^2}) \times C_q
            local a, b, ii, qq, s, t, coll, G;
            a := ZmodnZObj(Int(Z(p)),p^2);
            if not a^(p-1) = ZmodnZObj(1,p^2) then b := a; else b := a+1; fi;
            s := Int(b^((p^2-p)/r));
            ii := s mod p;
            qq := (s - ii)/p;
            coll := FromTheLeftCollector(4);
            SetRelativeOrder(coll, 1, r);
            SetRelativeOrder(coll, 2, p);
            SetRelativeOrder(coll, 3, p);
            SetRelativeOrder(coll, 4, q);
            SetPower(coll, 2, [3, 1]);
            SetConjugate(coll, 2, 1, [2, ii, 3, qq]);
            SetConjugate(coll, 3, 1, [3, ii]);
            G := PcpGroupByCollector(coll);
          return PcpGroupToPcGroup(G:FreeGroupFamilyType:="syllable");
        end;
        ##
        if (p - 1) mod r = 0 then
          Add(list, G1(p, q, r));
        fi;
        ##
        G2 := function(p, q, r) ## r | (p - 1) and G \cong (C_r \ltimes C_p) \times (C_p \times C_q)
          local a, b, s, t, coll, G;
            a := Z(p);
            s := Int(a^((p-1)/r));
            coll := FromTheLeftCollector(4);
            SetRelativeOrder(coll, 1, r);
            SetRelativeOrder(coll, 2, p);
            SetRelativeOrder(coll, 3, p);
            SetRelativeOrder(coll, 4, q);
            SetConjugate(coll, 2, 1, [2, s]);
            G := PcpGroupByCollector(coll);
          return PcpGroupToPcGroup(G:FreeGroupFamilyType:="syllable");
        end;
        ##
        if (p - 1) mod r = 0 then
          Add(list, G2(p, q, r));
        fi;
        ##
        G3 := function(p, q, r, k) ## r | (p - 1) and G \cong (C_r \ltimes C_p^2) \times C_q
          local a, b, c, ii, qq, s, t, coll, G;
            a := Z(p);
            b := Z(r);
            c := Int(b^k);
            s := Int(a^((p-1)/r));
            t := Int(a^(c*(p-1)/r));
            coll := FromTheLeftCollector(4);
            SetRelativeOrder(coll, 1, r);
            SetRelativeOrder(coll, 2, p);
            SetRelativeOrder(coll, 3, p);
            SetRelativeOrder(coll, 4, q);
            SetConjugate(coll, 2, 1, [2, s]);
            SetConjugate(coll, 3, 1, [3, t]);
            G := PcpGroupByCollector(coll);
          return PcpGroupToPcGroup(G:FreeGroupFamilyType:="syllable");
        end;
        ##
        if (p - 1) mod r = 0 and r > 2 then
          for i in [0..(r - 1)/2] do
           Add(list, G3(p, q, r, i));
          od;
        fi;
        ##
        if r = 2 then Error("r must be a prime greater than q");
        fi;
        ##
        G4 := function(p, q, r) ## r | (p + 1) and G \cong (C_r \ltimes C_p^2) \times C_q
          local matr, coll, G;
            matr := msg.QthRootGL2P(p, r);
            coll := FromTheLeftCollector(4);
            SetRelativeOrder(coll, 1, r);
            SetRelativeOrder(coll, 2, p);
            SetRelativeOrder(coll, 3, p);
            SetRelativeOrder(coll, 4, q);
            SetConjugate(coll, 2, 1, [2, Int(matr[1][1]), 3, Int(matr[2][1])]);
            SetConjugate(coll, 3, 1, [2, Int(matr[1][2]), 3, Int(matr[2][2])]);
            G := PcpGroupByCollector(coll);
          return PcpGroupToPcGroup(G:FreeGroupFamilyType:="syllable");
        end;
        ##
        if (p + 1) mod r = 0 and r > 2 then
          Add(list, G4(p, q, r));
        fi;
        ##
        if r = 2 then Error("r must be a prime greater than q");
        fi;
        ##
        return list;
      end;
############
      Append(s, case4(p, q, r));
############ case 5: nonabelian and Fitting subgroup has order p^2r -- q | (p - 1)(r - 1) or q | (p + 1)(r - 1)
      case5 := function(p, q, r)
        local list, G1, G2, G3, G4, G5, G6, G7, G8, G9, G10, i, j;
          list := [];
          G1 := function(p, q, r) ## q \mid (r - 1) and G \cong (C_q \ltimes C_r) \times C_{p^2}
            local a, b, s, coll, G;
              a := Z(r);
              b := a^((r-1)/q);
              coll := FromTheLeftCollector(4);
              SetRelativeOrder(coll, 1, q);
              SetRelativeOrder(coll, 2, r);
              SetRelativeOrder(coll, 3, p);
              SetRelativeOrder(coll, 4, p);
              SetPower(coll, 3, [4, 1]);
              SetConjugate(coll, 2, 1, [2, Int(b)]);
              G := PcpGroupByCollector(coll);
            return PcpGroupToPcGroup(G:FreeGroupFamilyType:="syllable");
          end;
          ##
          if (r - 1) mod q = 0 then
            Add(list, G1(p, q, r));
          fi;
          ##
          G2 := function(p, q, r) ## q \mid (p - 1) and G \cong (C_q \ltimes C_{p^2}) \times C_r
            local a, b, ii, qq, s, t, coll, G;
              a := ZmodnZObj(Int(Z(p)),p^2);
              if not a^(p-1) = ZmodnZObj(1,p^2) then b := a; else b := a+1; fi;
              s := Int(b^((p^2-p)/q));
              ii := s mod p;
              qq := (s - ii)/p;
              coll := FromTheLeftCollector(4);
              SetRelativeOrder(coll, 1, q);
              SetRelativeOrder(coll, 2, p);
              SetRelativeOrder(coll, 3, p);
              SetRelativeOrder(coll, 4, r);
              SetPower(coll, 2, [3, 1]);
              SetConjugate(coll, 2, 1, [2, ii, 3, qq]);
              SetConjugate(coll, 3, 1, [3, ii]);
              G := PcpGroupByCollector(coll);
            return PcpGroupToPcGroup(G:FreeGroupFamilyType:="syllable");
          end;
          ##
          if (p - 1) mod q = 0 then
            Add(list, G2(p, q, r));
          fi;
          ##
          G3 := function(p, q, r, k) ## q \mid (p - 1) and G \cong C_q \ltimes (C_{p^2} \times C_r)
            local a, b, c, ii, qq, s, t, coll, G;
              a := ZmodnZObj(Int(Z(p)),p^2);
              if not a^(p-1) = ZmodnZObj(1,p^2) then b := a; else b := a+1; fi;
              s := Int(b^((p^2-p)/q));
              ii := s mod p;
              qq := (s - ii)/p;
              c := Z(r);
              t := c^(k*(r-1)/q);
              coll := FromTheLeftCollector(4);
              SetRelativeOrder(coll, 1, q);
              SetRelativeOrder(coll, 2, p);
              SetRelativeOrder(coll, 3, p);
              SetRelativeOrder(coll, 4, r);
              SetPower(coll, 2, [3, 1]);
              SetConjugate(coll, 2, 1, [2, ii, 3, qq]);
              SetConjugate(coll, 3, 1, [3, ii]);
              SetConjugate(coll, 4, 1, [4, Int(t)]);
              G := PcpGroupByCollector(coll);
            return PcpGroupToPcGroup(G:FreeGroupFamilyType:="syllable");
          end;
          ##
          if (p - 1) mod q = 0 and (r - 1) mod q = 0 then
            for i in [1..(q-1)] do
              Add(list, G3(p, q, r, i));
            od;
          fi;
          ##
          G4 := function(p, q, r) ## q \mid (p - 1) and G \cong (C_q \ltimes C_p) \times C_p \times C_r
            local a, b, s, t, coll, G;
              a := Z(p);
              s := Int(a^((p-1)/q));
              coll := FromTheLeftCollector(4);
              SetRelativeOrder(coll, 1, q);
              SetRelativeOrder(coll, 2, p);
              SetRelativeOrder(coll, 3, p);
              SetRelativeOrder(coll, 4, r);
              SetConjugate(coll, 2, 1, [2, s]);
              G := PcpGroupByCollector(coll);
            return PcpGroupToPcGroup(G:FreeGroupFamilyType:="syllable");
          end;
          ##
          if (p - 1) mod q = 0 then
            Add(list, G4(p, q, r));
          fi;
          ##
          G5 := function(p, q, r, k) ## q | (p - 1) and G \cong (C_q \ltimes C_p^2) \times C_r
            local a, b, c, s, t, coll, G;
              a := Z(p);
              b := Z(q);
              c := Int(b^k);
              s := Int(a^((p-1)/q));
              t := Int(a^(c*(p-1)/q));
              coll := FromTheLeftCollector(4);
              SetRelativeOrder(coll, 1, q);
              SetRelativeOrder(coll, 2, p);
              SetRelativeOrder(coll, 3, p);
              SetRelativeOrder(coll, 4, r);
              SetConjugate(coll, 2, 1, [2, s]);
              SetConjugate(coll, 3, 1, [3, t]);
              G := PcpGroupByCollector(coll);
            return PcpGroupToPcGroup(G:FreeGroupFamilyType:="syllable");
          end;
          ##
          if (p - 1) mod q = 0 and q > 2 then
            for i in [0..(q - 1)/2] do
             Add(list, G5(p, q, r, i));
            od;
          fi;
          ##
          if (p - 1) mod q = 0 and q = 2 then
            Add(list, G5(p, q, r, 0));
          fi;
          ##
          G6 := function(p, q, r, k) ## q | (p + 1), and G \cong C_q \ltimes (C_r \times C_p^2)
            local matq, coll, a, b, c, G;
              matq := msg.QthRootGL2P(p, q);
              b := matq^k;
              coll := FromTheLeftCollector(4);
              SetRelativeOrder(coll, 1, q);
              SetRelativeOrder(coll, 2, r);
              SetRelativeOrder(coll, 3, p);
              SetRelativeOrder(coll, 4, p);
              SetConjugate(coll, 3, 1, [3, Int(b[1][1]), 4, Int(b[2][1])]);
              SetConjugate(coll, 4, 1, [3, Int(b[1][2]), 4, Int(b[2][2])]);
              G := PcpGroupByCollector(coll);
            return PcpGroupToPcGroup(G:FreeGroupFamilyType:="syllable");
          end;
          ##
          if (p + 1) mod q = 0 and q > 2 then
            for i in [1..(q-1)/2] do
              Add(list, G6(p, q, r, i));
            od;
          fi;
          ##
          G7 := function(p, q, r) ## q | (r - 1), and G \cong (C_q \ltimes C_r) \times C_p^2
            local a, b, c, s, t, coll, G;
              a := Z(r);
              b := a^((r - 1)/q);
              coll := FromTheLeftCollector(4);
              SetRelativeOrder(coll, 1, q);
              SetRelativeOrder(coll, 2, r);
              SetRelativeOrder(coll, 3, p);
              SetRelativeOrder(coll, 4, p);
              SetConjugate(coll, 2, 1, [2, Int(b)]);
              G := PcpGroupByCollector(coll);
            return PcpGroupToPcGroup(G:FreeGroupFamilyType:="syllable");
          end;
          ##
          if (r - 1) mod q = 0 then
            Add(list, G7(p, q, r));
          fi;
          ##
          G8 := function(p, q, r, l) ## q | (r - 1), q | (p - 1), and G \cong C_q \ltimes (C_r \times C_p^2)
            local a, b, c, d, s, t, coll, G;
              a := Z(r);
              b := Z(p);
              s := a^((r-1)/q);
              t := b^((p-1)/q);
              coll := FromTheLeftCollector(4);
              SetRelativeOrder(coll, 1, q);
              SetRelativeOrder(coll, 2, r);
              SetRelativeOrder(coll, 3, p);
              SetRelativeOrder(coll, 4, p);
              SetConjugate(coll, 2, 1, [2, Int(s^l)]);
              SetConjugate(coll, 3, 1, [3, Int(t)]);
              G := PcpGroupByCollector(coll);
            return PcpGroupToPcGroup(G:FreeGroupFamilyType:="syllable");
          end;
          ##
          if (r - 1) mod q = 0 and (p - 1) mod q = 0 then
            for i in [1..q-1] do
              Add(list, G8(p, q, r, i));
            od;
          fi;
          ##
          G9 := function(p, q, r, l, k) ## q | (r - 1), q | (p - 1), and G \cong C_q \ltimes (C_r \times C_p^2)
            local a, b, c, d, e, s, t, coll, G;
              a := Z(r);
              b := Z(p);
              c := Z(q);
              d := Int(c^k);
              e := Int(c^l);
              s := a^((r-1)/q);
              t := b^((p-1)/q);
              coll := FromTheLeftCollector(4);
              SetRelativeOrder(coll, 1, q);
              SetRelativeOrder(coll, 2, r);
              SetRelativeOrder(coll, 3, p);
              SetRelativeOrder(coll, 4, p);
              SetConjugate(coll, 2, 1, [2, Int(s^l)]);
              SetConjugate(coll, 3, 1, [3, Int(t)]);
              SetConjugate(coll, 4, 1, [4, Int(t^d)]);
              G := PcpGroupByCollector(coll);
            return PcpGroupToPcGroup(G:FreeGroupFamilyType:="syllable");
          end;
          ##
          if (r - 1) mod q = 0 and (p - 1) mod q = 0 and q > 2 then
            for i in [1..q-1] do
              for j in [0..(q-1)/2] do
                if not i = j then
                  Add(list, G9(p, q, r, i, j));
                fi;
              od;
            od;
          fi;
          ##
          if q = 2 then
            Add(list, G9(p, q, r, 1, 0));
          fi;
          ##
          G10 := function(p, q, r, k) ## q | (r - 1), q | (p + 1), and G \cong C_q \ltimes (C_r \times C_p^2)
            local matq, coll, a, b, c, G;
              matq := msg.QthRootGL2P(p, q);
              a := Z(r);
              b := matq^k;
              c := a^((r - 1)/q);
              coll := FromTheLeftCollector(4);
              SetRelativeOrder(coll, 1, q);
              SetRelativeOrder(coll, 2, r);
              SetRelativeOrder(coll, 3, p);
              SetRelativeOrder(coll, 4, p);
              SetConjugate(coll, 2, 1, [2, Int(c)]);
              SetConjugate(coll, 3, 1, [3, Int(b[1][1]), 4, Int(b[2][1])]);
              SetConjugate(coll, 4, 1, [3, Int(b[1][2]), 4, Int(b[2][2])]);
              G := PcpGroupByCollector(coll);
            return PcpGroupToPcGroup(G:FreeGroupFamilyType:="syllable");
          end;
          ##
          if (p + 1) mod q = 0 and (r - 1) mod q = 0 and q > 2 then
            for i in [1..(q-1)/2] do
              Add(list, G10(p, q, r, i));
            od;
          fi;
          ##
          return list;
        end;
############
      Append(s, case5(p, q, r));
############ case 6: nonabelian and Fitting subgroup has order pr -- q | (p - 1)(r - 1) and p | (r - 1)
      case6 := function(p, q, r)
        local list, G1, G2, G3, G4, i, j;
          list := [];
          G1 := function(p, q, r) ## q | (r - 1), p | (r - 1), and G \cong (C_p \times C_q) \ltimes C_r \times C_p
            local a, b, c, coll, G;
              a := Z(r);
              b := a^((r-1)/p);
              c := a^((r-1)/q);
              coll := FromTheLeftCollector(4);
              SetRelativeOrder(coll, 1, p);
              SetRelativeOrder(coll, 2, q);
              SetRelativeOrder(coll, 3, p);
              SetRelativeOrder(coll, 4, r);
              SetConjugate(coll, 4, 1, [4, Int(b)]);
              SetConjugate(coll, 4, 2, [4, Int(c)]);
              G := PcpGroupByCollector(coll);
            return PcpGroupToPcGroup(G:FreeGroupFamilyType:="syllable");
          end;
          ##
          if (r - 1) mod p = 0 and (r - 1) mod q = 0 then
            Add(list, G1(p, q, r));
          fi;
          ##
          G2 := function(p, q, r) ## q | (r - 1), p | (r - 1), and G \cong (C_p \times C_q) \ltimes C_r \times C_p
            local a, b, c, coll, G;
              a := Z(r);
              b := a^((r-1)/p);
              c := a^((r-1)/q);
              coll := FromTheLeftCollector(4);
              SetRelativeOrder(coll, 1, p);
              SetRelativeOrder(coll, 2, q);
              SetRelativeOrder(coll, 3, p);
              SetRelativeOrder(coll, 4, r);
              SetPower(coll, 1, [3, 1]);
              SetConjugate(coll, 4, 1, [4, Int(b)]);
              SetConjugate(coll, 4, 2, [4, Int(c)]);
              G := PcpGroupByCollector(coll);
            return PcpGroupToPcGroup(G:FreeGroupFamilyType:="syllable");
          end;
          ##
          if (r - 1) mod p = 0 and (r - 1) mod q = 0 then
            Add(list, G2(p, q, r));
          fi;
          ##
          G3 := function(p, q, r) ## q | (p - 1), p | (r - 1), and G \cong (C_p \ltimes C_r) \times (C_q \ltimes C_p)
            local a, b, c, d, coll, G;
              a := Z(r);
              b := Z(p);
              c := a^((r-1)/p);
              d := b^((p-1)/q);
              coll := FromTheLeftCollector(4);
              SetRelativeOrder(coll, 1, p);
              SetRelativeOrder(coll, 2, r);
              SetRelativeOrder(coll, 3, q);
              SetRelativeOrder(coll, 4, p);
              SetConjugate(coll, 2, 1, [2, Int(c)]);
              SetConjugate(coll, 4, 3, [4, Int(d)]);
              G := PcpGroupByCollector(coll);
            return PcpGroupToPcGroup(G:FreeGroupFamilyType:="syllable");
          end;
          ##
          if (r - 1) mod p = 0 and (p - 1) mod q = 0 then
            Add(list, G3(p, q, r));
          fi;
          ##
          G4 := function(p, q, r, k) ## q | (p - 1), p | (r - 1), q | (r - 1), and G \cong (C_p \times C_q) \ltimes (C_r \times C_p)
            local a, b, c, d, e, coll, G;
              a := Z(r);
              b := Z(p);
              c := a^((r-1)/p);
              d := a^((r-1)/q);
              e := b^((p-1)/q);
              coll := FromTheLeftCollector(4);
              SetRelativeOrder(coll, 1, p);
              SetRelativeOrder(coll, 2, q);
              SetRelativeOrder(coll, 3, r);
              SetRelativeOrder(coll, 4, p);
              SetConjugate(coll, 3, 1, [3, Int(c)]);
              SetConjugate(coll, 3, 2, [3, Int(d)]);
              SetConjugate(coll, 4, 2, [4, Int(e^k)]);
              G := PcpGroupByCollector(coll);
            return PcpGroupToPcGroup(G:FreeGroupFamilyType:="syllable");
          end;
          ##
          if (r - 1) mod p = 0 and (p - 1) mod q = 0 and (r - 1) mod q = 0 then
            for i in [1..q-1] do
              Add(list, G4(p, q, r, i));
            od;
          fi;
          ##
          return list;
        end;
############
        Append(s, case6(p, q, r));
############ case 7: nonabelian and Fitting subgroup has order pqr -- p | (r - 1)(q - 1)
        case7 := function(p, q, r)
          local list, G1, G2, G3, i, j;
          list := [];
          G1 := function(p, q, r) ## P \cong C_{p^2}, p | (r - 1) and G \cong (C_{p^2} \ltimes C_r) \times C_q
            local a, b, c, coll, G;
              a := Z(r);
              b := a^((r-1)/p);
              coll := FromTheLeftCollector(4);
              SetRelativeOrder(coll, 1, p);
              SetRelativeOrder(coll, 2, p);
              SetPower(coll, 1, [2, 1]);
              SetRelativeOrder(coll, 3, r);
              SetRelativeOrder(coll, 4, q);
              SetConjugate(coll, 3, 1, [3, Int(b)]);
              G := PcpGroupByCollector(coll);
            return PcpGroupToPcGroup(G:FreeGroupFamilyType:="syllable");
          end;
          ##
          if (r - 1) mod p = 0 then
            Add(list, G1(p, q, r));
          fi;
          ##
          G2 := function(p, q, r) ## P \cong C_{p^2}, p | (q - 1) and G \cong (C_{p^2} \ltimes C_q) \times C_r
            local a, b, c, coll, G;
              a := Z(q);
              b := a^((q-1)/p);
              coll := FromTheLeftCollector(4);
              SetRelativeOrder(coll, 1, p);
              SetRelativeOrder(coll, 2, p);
              SetPower(coll, 1, [2, 1]);
              SetRelativeOrder(coll, 3, q);
              SetRelativeOrder(coll, 4, r);
              SetConjugate(coll, 3, 1, [3, Int(b)]);
              G := PcpGroupByCollector(coll);
            return PcpGroupToPcGroup(G:FreeGroupFamilyType:="syllable");
          end;
          ##
          if (q - 1) mod p = 0 then
            Add(list, G2(p, q, r));
          fi;
          ##
          G3 := function(p, q, r, k) ## P \cong C_{p^2} and G \cong C_{p^2} \ltimes (C_q \times C_r)
            local a, b, c, d, coll, G;
              a := Z(q);
              b := Z(r);
              c := a^((q-1)/p);
              d := b^(k*(r-1)/p);
              coll := FromTheLeftCollector(4);
              SetRelativeOrder(coll, 1, p);
              SetRelativeOrder(coll, 2, p);
              SetPower(coll, 1, [2, 1]);
              SetRelativeOrder(coll, 3, q);
              SetRelativeOrder(coll, 4, r);
              SetConjugate(coll, 3, 1, [3, Int(c)]);
              SetConjugate(coll, 4, 1, [4, Int(d)]);
              G := PcpGroupByCollector(coll);
            return PcpGroupToPcGroup(G:FreeGroupFamilyType:="syllable");
          end;
          ##
          if (r - 1) mod p = 0 and (q - 1) mod p = 0 then
            for i in [1..p-1] do
              Add(list, G3(p, q, r, i));
            od;
          fi;
          ##
          return list;
        end;
############
        Append(s, case7(p, q, r));
############  case 8: nonabelian and Fitting subgroup has order pqr -- p | (r - 1)(q - 1)
        case8 := function(p, q, r)
          local list, G1, G2, G3, i, j;
          list := [];
          G1 := function(p, q, r) ## P \cong C_p^2, p | (r - 1) and G \cong C_p \times (C_p \ltimes C_r) \times C_q
            local a, b, c, d, coll, G;
              a := Z(r);
              b := a^((r-1)/p);
              coll := FromTheLeftCollector(4);
              SetRelativeOrder(coll, 1, p);
              SetRelativeOrder(coll, 2, p);
              SetRelativeOrder(coll, 3, q);
              SetRelativeOrder(coll, 4, r);
              SetConjugate(coll, 4, 1, [4, Int(b)]);
              G := PcpGroupByCollector(coll);
            return PcpGroupToPcGroup(G:FreeGroupFamilyType:="syllable");
          end;
          ##
          if (r - 1) mod p = 0 then
            Add(list, G1(p, q, r));
          fi;
          ##
          G2 := function(p, q, r) ## P \cong C_p^2, p | (r - 1) and G \cong C_p \times (C_p \ltimes C_r) \times C_q
            local a, b, c, d, coll, G;
              a := Z(q);
              b := a^((q-1)/p);
              coll := FromTheLeftCollector(4);
              SetRelativeOrder(coll, 1, p);
              SetRelativeOrder(coll, 2, p);
              SetRelativeOrder(coll, 3, q);
              SetRelativeOrder(coll, 4, r);
              SetConjugate(coll, 3, 1, [3, Int(b)]);
              G := PcpGroupByCollector(coll);
            return PcpGroupToPcGroup(G:FreeGroupFamilyType:="syllable");
          end;
          ##
          if (q - 1) mod p = 0 then
            Add(list, G2(p, q, r));
          fi;
          ##
          G3 := function(p, q, r, k) ## P \cong C_{p^2} and G \cong C_{p^2} \ltimes (C_q \times C_r)
            local a, b, c, d, coll, G;
              a := Z(q);
              b := Z(r);
              c := a^((q-1)/p);
              d := b^(k*(r-1)/p);
              coll := FromTheLeftCollector(4);
              SetRelativeOrder(coll, 1, p);
              SetRelativeOrder(coll, 2, p);
              SetRelativeOrder(coll, 3, q);
              SetRelativeOrder(coll, 4, r);
              SetConjugate(coll, 3, 1, [3, Int(c)]);
              SetConjugate(coll, 4, 1, [4, Int(d)]);
              G := PcpGroupByCollector(coll);
            return PcpGroupToPcGroup(G:FreeGroupFamilyType:="syllable");
          end;
          ##
          if (r - 1) mod p = 0 and (q - 1) mod p = 0 then
            for i in [1..p-1] do
              Add(list, G3(p, q, r, i));
            od;
          fi;
          ##
          return list;
        end;
############
        Append(s, case8(p, q, r));
############
        if n = 60 then
          Add(s, AlternatingGroup(5));
        fi;
############
        return s;
      end);

######################################################
NumberGroupsP2QR := function(n)
  local s, fac, primefac, p, q, r, m;
    s := [];
    fac := Factors(n);
    if not Length(fac) = 4 or not Length(Collected(fac)) = 3 then
      Error("Argument must be of the form of p^2qr"); fi;
      primefac := function(n)
        local i, j, tmp;
          tmp := [];
          for i in Collected(fac) do
            if i[2] = 2 then j := i[1];
            elif i[2] = 1 then Add(tmp, i[1]);
            fi;
          od;
          Sort(tmp);
          Add(tmp, j);
          return tmp;
        end;
    p := primefac(n)[3]; q := primefac(n)[1]; r := primefac(n)[2];

    if n = 60 then m := 13;
    fi;
    if q = 2 then
      m := 10 + (2*r + 7)*deltaDivisibility((p - 1), r) + 3*deltaDivisibility((p + 1), r) + 6*deltaDivisibility((r - 1), p) + 2*deltaDivisibility((r - 1), (p^2));
    fi;
    if q > 2 and not n = 60 then
      m := 2 + (p^2 - p)*deltaDivisibility((q - 1), (p^2))*deltaDivisibility((r - 1), (p^2))
      + (p - 1)*(deltaDivisibility((q - 1), (p^2))*deltaDivisibility((r - 1),p) + deltaDivisibility((r - 1), (p^2))*deltaDivisibility((q - 1), p) + 2*deltaDivisibility((r - 1), p)*deltaDivisibility((q - 1), p))
      + (q - 1)*(q + 4)*deltaDivisibility((p - 1), q)*deltaDivisibility((r - 1), q)/2
      + (q - 1)*(deltaDivisibility((p + 1), q)*deltaDivisibility((r - 1), q) + deltaDivisibility((p - 1), q) + deltaDivisibility((p - 1), (q*r)) + 2*deltaDivisibility((r - 1), (p*q))*deltaDivisibility((p - 1), q))/2
      + (q*r + 1)*deltaDivisibility((p - 1), (q*r))/2
      + (r + 5)*deltaDivisibility((p - 1), r)*(1 + deltaDivisibility((p - 1), q))/2
      + deltaDivisibility((p^2 - 1), (q*r)) + 2*deltaDivisibility((r - 1), (p*q)) + deltaDivisibility((r - 1), p)*deltaDivisibility((p - 1), q) + deltaDivisibility((r - 1), (p^2*q))
      + deltaDivisibility((r - 1), p)*deltaDivisibility((q - 1), p) + 2*deltaDivisibility((q - 1), p) + 3*deltaDivisibility((p - 1), q) + 2*deltaDivisibility((r - 1), p)
      + 2*deltaDivisibility((r - 1), q) + deltaDivisibility((r - 1), (p^2)) + deltaDivisibility((p + 1), r) + deltaDivisibility((p + 1), q);
    fi;
    return m;
  end;
######################################################
